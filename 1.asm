...
LoadUTF8ButtonTitle db "Load UTF-8", 0
...
WindowCreate:
    ...
    push    OFFSET TextBoxClassName ; Class name
    push    0                       ; Extended window style
    call    CreateWindowExA
    mov     hEdit, eax              ; Save the handle to the text box
    ...
    ; Create a load UTF-8 button
    push    NULL                    ; No additional parameters
    push    hInstance               ; Instance handle
    push    1004                    ; Control ID
    push    hWnd                    ; Parent window handle
    push    30                      ; Height
    push    80                      ; Width
    push    330                      ; y position
    push    10                      ; x position
    push    WS_CHILD or WS_VISIBLE or BS_PUSHBUTTON ; Window style
    push    OFFSET LoadUTF8ButtonTitle  ; Window name
    push    OFFSET ButtonClassName  ; Class name
    push    0                       ; Extended window style
    call    CreateWindowExA
    ...
    mov     hLoadUTF8Button, eax    ; Save the handle to the load UTF-8 button
...
CommandHandler:
    ...
    je      LoadButtonClick
    cmp     eax, 1004               ; Check if the load UTF-8 button was clicked
    je      LoadUTF8ButtonClick
    jmp     NotButtonClick

SaveButtonClick:
    ; Get text from the text box using GetDlgItemTextA
    lea     eax, buffer             ; Load effective address of buffer
    push    256                     ; Size of the buffer
    push    eax
    push    1001                    ; Control ID of the text box
...
LoadButtonClick:
    ...
    ; Set text to the text box using SetDlgItemTextA
    lea     eax, buffer             ; Load effective address of buffer
    push    eax
    push    1001                    ; Control ID of the text box
    push    hWnd                    ; Handle to the parent window
...
LoadUTF8ButtonClick:
    ; Load text from file in UTF-8 format
    push    GENERIC_READ            ; Desired access
    push    FILE_SHARE_READ         ; Share mode
    push    NULL                    ; Security attributes
    push    OPEN_EXISTING           ; Creation disposition
    ...
    lea     eax, buffer             ; Load effective address of buffer
    push    eax                     ; UTF-8 string
    push    -1                      ; Calculate the length of the UTF-8 string
    push    NULL                    ; Reserved, must be NULL
    push    CP_UTF8                 ; Code page for UTF-8
    call    MultiByteToWideChar

    ; Allocate memory for the wide string
    mov     ecx, eax                ; Length of the wide string
    shl     ecx, 1                  ; Multiply by 2 (size of WCHAR)
    push    ecx                     ; Size of the memory block
    call    GlobalAlloc
    mov     esi, eax                ; Save the pointer to the wide string

    ; Convert UTF-8 to wide string
    lea     eax, buffer             ; Load effective address of buffer
    push    esi                     ; Pointer to the wide string
    push    ecx                     ; Size of the wide string buffer
    push    -1                      ; Calculate the length of the UTF-8 string
    push    eax                     ; UTF-8 string
    push    NULL                    ; Reserved, must be NULL
    push    CP_UTF8                 ; Code page for UTF-8
    ...
    push    esi                     ; Pointer to the wide string
    push    -1                      ; Calculate the length of the wide string
    push    NULL                    ; Reserved, must be NULL
    push    CP_ACP                  ; Code page for ANSI
    call    WideCharToMultiByte

    ; Allocate memory for the ANSI string
    mov     ecx, eax                ; Length of the ANSI string
    push    ecx                     ; Size of the memory block
    call    GlobalAlloc
    mov     edi, eax                ; Save the pointer to the ANSI string

    ; Convert wide string to ANSI
    push    edi                     ; Pointer to the ANSI string
    push    ecx                     ; Size of the ANSI string buffer
    push    -1                      ; Calculate the length of the wide string
    push    esi                     ; Pointer to the wide string
    push    NULL                    ; Reserved, must be NULL
    push    CP_ACP                  ; Code page for ANSI
    ...
    ; Free the memory for the wide string
    push    esi                     ; Pointer to the wide string
    call    GlobalFree
    ...
    ; Free the memory for the ANSI string
    push    edi                     ; Pointer to the ANSI string
    call    GlobalFree
